...
## 9. 新規プロジェクトの追加と既存プロジェクトの更新

### 新規プロジェクト「生成AIを社内限定で安全に利用するためのAzure閉域ネットワーク基盤」の追加

`portfolio.html`ファイル内の`<div class="portfolio-container">`内に、以下のテンプレートをコピーして追加します。`project-card`の`fade-in delay-X`の`X`は、既存のプロジェクトカードの`delay`値に続くように調整してください。

```html
<div class="project-card fade-in delay-1">
  <div class="project-header">
    <h2 class="project-title">生成AIを社内限定で安全に利用するためのAzure閉域ネットワーク基盤</h2>
  </div>
  <img src="images/アーキ図（Azure）.png" alt="生成AI向けAzure閉域ネットワーク基盤のアーキテクチャ図" class="project-image">
  <div class="project-content">
    <p>生成AIを社内限定で安全に利用するためのAzure閉域ネットワーク基盤を設計・構築。Private Endpoint + VNet統合によりAIサービス群を内部接続し、WAF・IP制限・NSGによる多層防御を実装。BicepによるIaC化とAzure DevOpsによるCI/CDを構築し、再現性と運用効率を向上。本番向け開発環境として、スケール制御と自動停止設計によりコスト最適化を実現。</p>
    
    <h3 class="project-subtitle">主な成果と取り組み</h3>
    <ul class="project-list">
      <li>AI Search / OpenAI / Storage / Key VaultをPrivate Endpoint接続し閉域化</li>
      <li>App / PEP / Appserviceでサブネット分離、通信経路を明確化</li>
      <li>Bastionではなく軽量のVMを踏み台サーバとして採用し自動シャットダウンでコスト削減</li>
      <li>Entra ID認証 + 拡張機能で踏み台VMをセキュア化</li>
    </ul>
    
    <h3 class="project-subtitle">使用技術</h3>
    <div class="tech-tags">
      <span class="tech-tag">Azure</span>
      <span class="tech-tag">Azure OpenAI Service</span>
      <span class="tech-tag">Azure AI Search</span>
      <span class="tech-tag">Azure Storage</span>
      <span class="tech-tag">Azure Key Vault</span>
      <span class="tech-tag">Azure Private Endpoint</span>
      <span class="tech-tag">Azure Virtual Network</span>
      <span class="tech-tag">Azure WAF</span>
      <span class="tech-tag">Azure NSG</span>
      <span class="tech-tag">Bicep (IaC)</span>
      <span class="tech-tag">Azure DevOps (CI/CD)</span>
...
### 既存プロジェクト「Kubernetes 監視基盤の設計・構築（AWS EKS）」の更新

`portfolio.html`ファイル内の該当プロジェクトの`<img>`タグを以下のように変更します。`images/アーキ図（監視基盤）.png`は、後ほど提供される画像ファイル名に置き換えてください。

```html
<img src="images/アーキ図（監視基盤）.png" alt="Kubernetes監視基盤のアーキテクチャ図" class="project-image">
```

また、プロジェクトの説明文、主な成果と取り組み、使用技術も最新の経歴メモに合わせて修正・追記してください。

### 既存プロジェクト「Microsoft Entra Connect サーバ構築（Azure IaaS）」の更新

`portfolio.html`ファイル内の該当プロジェクトの説明文、主な成果と取り組み、使用技術を最新の経歴メモに合わせて修正・追記してください。

### その他のプロジェクトの更新

`portfolio.html`ファイル内の各プロジェクトの説明文、主な成果と取り組み、使用技術を最新の経歴メモに合わせて修正・追記してください。
